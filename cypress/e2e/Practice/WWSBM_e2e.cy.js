...
describe('MYWWSBM Verification Workflow', function () {
    ...
    it('System Admin File Upload and Assign to Verification', function () {
        ...
        // URL
        cy.visit("https://testing.mywwsbm.com/internal"); //Testing Url
        cy.wait(10000);

        // Login details
        cy.get('#basic_email').type('sa.stt@mywwsbm.com');  // Username
        cy.get('#basic_password').type('Wonderwsbm@24');   // Password
        ...
        cy.contains('System Administrator').click(); //dropdown
        ...
        //file upload
        cy.contains('Import Data for Verification').click(); //upload button
        cy.get('#project').click()  //select category
        cy.contains('.ant-select-item-option-content', 'Test Facility 1').click(); //facility select

        const fileName = 'alpha 6.xlsx'; // file in fixtures
        ...
        cy.contains('Verification Assign').click(); //Assigning to Data Intergrity Speciliast
        cy.contains('Assign by Split').click(); //Assigning by split method
        cy.get('.ant-select-selection-overflow').click(); //pop up to select members
        cy.contains('Eligible and Benefits Verifier').click(); //Selecting user
        ...
        cy.contains('Log Out').click(); //Log out button
